...
import { baseHeader, makeBlock, makePortal } from './batch.helpers';

describe('coalesceBatchRequests (tx)', () => {
  it('coalesces block + tx-by-index requests into one stream', async () => {
    const config = loadConfig({
      SERVICE_MODE: 'single',
      PORTAL_DATASET: 'ethereum-mainnet',
      PORTAL_CHAIN_ID: '1'
    });
    const calls: Array<{ fullTx: boolean; allBlocks: boolean }> = [];
    const portal = makePortal({
      streamBlocks: async (_base: string, _finalized: boolean, req: { fields: { block: Record<string, boolean>; transaction: Record<string, boolean> } }) => {
        calls.push({
          fullTx: Boolean(req.fields.transaction.input),
          allBlocks: Boolean(req.fields.block.stateRoot)
        });
    ...
    expect(calls[0]).toEqual({ fullTx: true, allBlocks: true });
  ...
  it('skips tx-by-index pending', async () => {
    const config = loadConfig({
      SERVICE_MODE: 'single',
      PORTAL_DATASET: 'ethereum-mainnet',
      PORTAL_CHAIN_ID: '1'
